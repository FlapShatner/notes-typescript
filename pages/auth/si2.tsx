    ...
    <div className=" w-5/6 sm:w-3/4 max-w-xl rounded-md border absolute left-1/2 -translate-x-1/2 top-48">
      <div className="flex flex-col py-10 px-6">
        ...
        <div className="bg-white rounded border border-gray-300 flex items-center justify-between">
          ...
          />
          <button onClick={() => emailSignIn()} className="py-2 px-4 bg-white text-gray-600 rounded-r border-l border-gray-200 hover:bg-gray-50 active:bg-gray-200 disabled:opacity-50 inline-flex items-center focus:outline-none text-lg">
            <FiSend />
          </button>
        </div>
        <span className="mt-4 text-center">or</span>
                <button onClick={() => signIn('google', { callbackUrl: localStorage.getItem("tempNote")? '/create': '/' })} className="bg-indigo-500 text-xs mt-4 sm:text-sm text-white py-2 hover:bg-indigo-600 active:bg-indigo-700">
